        ...
        if test_ID in [0, 2]:
            for depth in range(1, m_depth+1):
                ...
                for num_tx in tx_range:
                    ...
                    time.sleep(100)
                with open("Analysis/tree_{}_depth_ETH_statistics_{}.csv".format(depth, unique), 'w') as f:
                    csvwriter = csv.writer(f)
                    csvwriter.writerow(fields)
                    csvwriter.writerows(rows)
                make_plots("Analysis/tree_{}_depth_ETH_statistics_{}.csv".format(depth, unique), yaxis='ETH')
                data[2**depth] = rows
            make_parametric_plots("Analysis/tree_parametric_ETH_statistics_{}_{}".format(m_depth, unique), "Peer(s)", data, fields, 'lower right')
        if test_ID in [0, 3]:
            for depth in range(1, m_depth+1):
                ...
                for num_tx in tx_range:
                    ...
                    time.sleep(100)
                with open("Analysis/faultlink_tree_{}_depth_ETH_statistics_{}.csv".format(depth, unique), 'w') as f:
                    csvwriter = csv.writer(f)
                    csvwriter.writerow(fields)
                    csvwriter.writerows(faultlink_rows)
                make_plots("Analysis/faultlink_tree_{}_depth_ETH_statistics_{}.csv".format(depth, unique), yaxis='ETH')
                faultlink_data[2**depth] = faultlink_rows
            make_parametric_plots("Analysis/faultlink_tree_parametric_ETH_statistics_{}_{}".format(m_depth, unique), "Peer(s)", faultlink_data, fields, 'lower right')
        os.system("sudo mn -c")
