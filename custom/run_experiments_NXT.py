            ...
            for depth in range(1, m_depth+1):
                ...
                for num_tx in tx_range:
                   ...
                   # os.system("sudo rm -f /home/mininet/Proc_Logs/* &>/dev/null")
                    ...
                    time.sleep(100)
                with open("/home/mininet/Analysis/tree_{}_depth_NXT_statistics_{}.csv".format(depth, unique), 'w') as f:
                    csvwriter = csv.writer(f)
                    csvwriter.writerow(fields)
                    csvwriter.writerows(rows)
                make_plots("/home/mininet/Analysis/tree_{}_depth_NXT_statistics_{}.csv".format(depth, unique), yaxis='custom')
                data[2**depth] = rows
            make_parametric_plots("/home/mininet/Analysis/tree_parametric_NXT_statistics_{}_{}".format(depth, unique), "Peer(s)", data, fields, 'lower right')
        if test_ID in [0, 3]:
            for depth in range(1, m_depth+1):
                ...
                for num_tx in tx_range:
                   ...
                   # os.system("sudo rm -f /home/mininet/Proc_Logs/* &>/dev/null")
                    #os.system("""sudo lsof | grep "(deleted)$" | sed -re 's/^\S+\s+(\S+)\s+\S+\s+([0-9]+).*/\1\/fd\/\2/' | while read file; do sudo bash -c ": > /proc/$file"; done""")
                    #result = functionInProcess(faultlink_tx_run, [depth, num_tx])
                    result = faultlink_tx_run(depth, num_tx)
                    faultlink_rows.append([num_tx, result[0], result[1]])
                    time.sleep(100)
                with open("/home/mininet/Analysis/faultlink_tree_{}_depth_NXT_statistics_{}.csv".format(depth, unique), 'w') as f:
                    csvwriter = csv.writer(f)
                    csvwriter.writerow(fields)
                    csvwriter.writerows(faultlink_rows)
                make_plots("/home/mininet/Analysis/faultlink_tree_{}_depth_NXT_statistics_{}.csv".format(depth, unique), yaxis='custom')            
                faultlink_data[2**depth] = faultlink_rows
            make_parametric_plots("/home/mininet/Analysis/faultlink_tree_parametric_NXT_statistics_{}_{}".format(depth, unique), "Peer(s)", faultlink_data, fields, 'lower right')
        os.system("sudo mn -c")
       # os.system("sudo /home/mininet/kill_monitors.sh &> /dev/null")
       # os.system("sudo rm -f /home/mininet/Proc_Logs/* &>/dev/null")
