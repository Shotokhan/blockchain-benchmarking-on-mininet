...
import json
from ETH_observingClient import observingClient
...
def run(depth, number_of_TX):
    ...
    script_name = "tree_{}_depth_{}_transactions_with_link_up_and_down_ETH".format(str(depth), str(number_of_TX))
    ...
    for i in range(n_bursts):
        ...
        if stop_it:
            break
        if elapsed >= (total_time // 4) and elapsed < (total_time // 2):
            try:
    ...
    total_traffic = tshark_total_traffic('/home/mininet/Captures/traffic_h1_{}_{}.pcap'.format(script_name, unique), ['udp', 'http', 'rtcp'])
